import { PhotographIcon, CalculatorIcon, ChartBarIcon, EmojiHappyIcon, XIcon } from "@heroicons/react/outline";
    ...
    >
      ...
      <div className="w-full divide-y divide-gray-700">
        <div className={`${selectedFile && "pb-7"} ${input && "space-y-2.5"}`}> 
            ...
            placeholder="what's happening?" className="w-full bg-transparent outline-none text-[#d9d9d9] text-lg placeholder-gray-500 tracking-wide  min-h-[50px]" />
        </div>
        {selectedFile && (
       <div className="relative">
          
       <div  className="absolute w-8 h-9 bg-[#15181c] hover:bg-[#272c26] bg-opacity-75 rounded-full flex items-center justify-center top-1 left-1 cursor-pointer" onClick={ ()=> setselectedFile(null)}>
...
<button disabled={!input.trim() && !selectedFile} onClick={sendPost} className="bg-[#1d9bf0] text-white rounded-full px-4 py-1.5 font-bold shadow-md hover:bg-[#1a8cd8] disabled:hover:bg-[#1d9bf0] disabled:opacity-50 disabled:cursor-default">
  Save
</button>
       </div>
      </div>
